...
if flag:
    ...
    for sub_module in alexnet.modules():
        ...
        if isinstance(sub_module, nn.Conv2d):
            ...
            kernel_grid = vutils.make_grid(
                kernel_all, normalize=True, scale_each=True, nrow=8
            )  # c, h, w
            writer.add_image(
                "{}_all".format(kernel_num), kernel_grid, global_step=kernel_num + 1
            )
...
if flag:
    ...
    img_transforms = transforms.Compose(
        [transforms.Resize((224, 224)), transforms.ToTensor(), norm_transform]
    )
